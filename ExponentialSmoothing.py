...
def formatTime(data):
    ...
    """
#     data["Time"] = pd.to_datetime(data["Time"], format='%H%M%S%f').dt.strftime('%H:%M:%S:%f')
    data["Time"] = pd.to_datetime(data["Time"], format='%H%M%S%f')
    data = data.rename(columns={"Time":"Date_Time"})
    return data

def tickResamplerAlignDatapoints(data, dataPoints, volume = False):
    """
...
def cleanData(datat, tradeType = None, timePeriod = None):
    """
...
def createdb(listdb, symbol, Metric):
    """
...
def get_log_return(df):
    """
        Generates log returns for given OHLC dataframe and appends
        log returns column to the dataframe
